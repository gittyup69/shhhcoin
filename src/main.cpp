// Copyright (c) 2020 Mitsuaki Nakamichi
...
#if defined(NDEBUG)
# error "Shhhcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6c6eddc647d9394df3f74dce1735d7ab8e4a1f08c660a486fd04699509492b18");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Shhhcoin: starting difficulty is 1 / 2^12
...
/** Fees smaller than this (in Mitsuaki) are considered zero fee (for transaction creation) */
int64 CTransaction::nMinTxFee = 500000;
/** Fees smaller than this (in Mitsuaki) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 500000;
...
const string strMessageMagic = "Shhhcoin Signed Message:\n";
...
{
    // Shhhcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 2635200 blocks, which will occur approximately every 4 months
    nSubsidy >>= (nHeight / 2635200); // Shhhcoin: 2.6352m blocks in ~4 months

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Shhhcoin: 2 days
static const int64 nTargetSpacing = .07 * 60; // Shhhcoin: 4.2 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Shhhcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Shhhcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3d9e270e3a3974b1ecc87732f230f8360c6da30d39862a558e721ac2b053a450");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "11/29/2020 Shhhcoin.xyz was purchased three days after Thanksgiving";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fe8597134f2f0918a7089c48b1ecc71b0789e84a0a5921e1e51db172d0fd0918fc8abab3a68bff56dbe8333cd20ed55cee163919366d90e8d1cff5d87e63c021") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1606704776;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087231649;

        if (fTestNet)
        {
            block.nTime    = 1606704783;
            block.nNonce   = 385281034;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x2abf316390748675020920c9b61fcdb66559ce42968112ed84f565d79a2344ce"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xad, 0xd5, 0xc6, 0xea }; // Shhhcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ShhhcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ShhhcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ShhhcoinMiner(CWallet *pwallet)
{
    printf("ShhhcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("shhhcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ShhhcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ShhhcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ShhhcoinMiner, pwallet));
}
